Object.assign(window.app, {
    ...
    dmEnsurePoller: function() {
        if (this.state.dmPollTimer) return;
        this.state.dmPollTimer = setInterval(async () => {
            try {
                if (this.state.dmPollBusy) return;
                const peerId = Number(this.state.dmPeerId || 0);
                if (!peerId) return;
                const modal = document.getElementById('messageModal');
                const inbox = document.getElementById('page-inbox');
                const modalActive = !!(modal && modal.classList && modal.classList.contains('active'));
                const inboxActive = !!(inbox && inbox.classList && inbox.classList.contains('active') && this.state.inboxTab === 'dm');
                if (!modalActive && !inboxActive) return;
                this.state.dmPollBusy = true;
                if (modalActive) await this.loadDMThread(peerId, { sinceOnly: true });
                else await this.loadInboxDMThread(peerId, { sinceOnly: true });
            } catch (_) {
            } finally {
                this.state.dmPollBusy = false;
            }
        }, 2500);
    },
    ...
    dmSearchPolicy: function() {
        const now = Date.now();
        const prev = this.__dmSearchPolicy || null;
        if (prev && prev.exp > now && prev.value) return prev.value;
        const src = (typeof window !== 'undefined' && window.__AISEEK_DM_SEARCH) ? window.__AISEEK_DM_SEARCH : {};
        const toInt = (v, d) => {
            const n = Number(v);
        ...
        const value = {
            ttlMs: clamp(toInt(src.ttlMs, 2500), 300, 20000),
            cap: clamp(toInt(src.cap, 60), 10, 400),
            timeoutMs: clamp(toInt(src.timeoutMs, 5000), 1000, 15000),
            retries: clamp(toInt(src.retries, 0), 0, 1),
        };
        this.__dmSearchPolicy = { value, exp: now + 10000 };
        return value;
    },
    ...
    dmSearchUsers: async function(query) {
        ...
        try {
            const policy = this.dmSearchPolicy();
            const url = `/api/v1/users/search-user?query=${encodeURIComponent(q)}`;
            const cache = (this.__dmSearchCache = this.__dmSearchCache || new Map());
            const ck = `dm:user:${q.toLowerCase()}`;
            const now = Date.now();
            const hit = cache.get(ck);
            let users = [];
            if (hit && hit.exp > now && Array.isArray(hit.users)) {
                users = hit.users;
                cache.delete(ck);
                cache.set(ck, hit);
            } else {
                const res = await this.apiRequest('GET', url, undefined, {
                    cancel_key: 'dm:search:users',
                    dedupe_key: url,
                    timeout_ms: Number(policy.timeoutMs || 5000),
                    retries: Number(policy.retries || 0),
                });
                if (!res.ok) throw new Error('search failed');
                users = await res.json();
                cache.delete(ck);
                cache.set(ck, { exp: now + Number(policy.ttlMs || 2500), users });
                while (cache.size > Number(policy.cap || 60)) {
                    const fk = cache.keys().next().value;
                    if (!fk) break;
                    cache.delete(fk);
                }
            }
            if (String(this.state.dmSearchQuery || '').trim() !== q) return;
            ...
            if ((!Array.isArray(filtered) || filtered.length === 0) && pending && pending.id) {
                ...
                try {
                    if (pending && pending.id) {
                        ...
                        const cur = Number(this.state.dmPeerId || 0);
                        const hit = cur ? (filtered || []).find(x => Number(x && x.id ? x.id : 0) === cur) : null;
                        if (hit && hit.id) {
                            this.openDMThread(Number(hit.id), { id: hit.id, nickname: hit.nickname, username: hit.username, avatar: hit.avatar, aiseek_id: hit.aiseek_id });
                        } else {
    ...
    loadDMThread: async function(peerId, opts = {}) {
        if (!this.state.user) return;
        const box = document.getElementById('dm_chat_box');
        if (!box) return;
        ...
        try {
            const pid = Number(peerId || 0);
            if (!this.state.dmLastMsgIdMap || typeof this.state.dmLastMsgIdMap !== 'object') this.state.dmLastMsgIdMap = {};
            if (!this.state.dmLastTsMap || typeof this.state.dmLastTsMap !== 'object') this.state.dmLastTsMap = {};
            const sinceOnly = !!(opts && opts.sinceOnly);
            const sinceId = sinceOnly ? Number(this.state.dmLastMsgIdMap[pid] || 0) : 0;
            const limit = sinceOnly ? 120 : 200;
            const url = `/api/v1/messages/list?user_id=${this.state.user.id}&other_id=${pid}&limit=${limit}${sinceId > 0 ? `&since_id=${sinceId}` : ''}`;
            ...
            if (!sinceOnly) box.innerHTML = '';

            let lastTs = sinceOnly ? Number(this.state.dmLastTsMap[pid] || 0) : 0;
            msgs.forEach(m => {
                ...
                box.appendChild(row);
                try {
                    const mid = Number(m && m.id ? m.id : 0);
                    if (mid && mid > Number(this.state.dmLastMsgIdMap[pid] || 0)) this.state.dmLastMsgIdMap[pid] = mid;
                    if (ts && ts > Number(this.state.dmLastTsMap[pid] || 0)) this.state.dmLastTsMap[pid] = ts;
                } catch (_) {
                }
            });
    ...
    openInboxDMThread: async function(peerId, peerMeta = null) {
        ...
        try {
            ...
            if (input && !input.__aiseek_bound_enter_send) {
                input.__aiseek_bound_enter_send = true;
                input.addEventListener('keydown', (ev) => {
                    ...
                    try { this.sendInboxDMMessage(); } catch (_) {}
                });
            }
        } catch (_) {
        }
    ...
    loadInboxDMThread: async function(peerId, opts = {}) {
        if (!this.state.user) return;
        ...
        try {
            const pid = Number(peerId || 0);
            if (!this.state.dmLastMsgIdMap || typeof this.state.dmLastMsgIdMap !== 'object') this.state.dmLastMsgIdMap = {};
            if (!this.state.inboxDmLastTsMap || typeof this.state.inboxDmLastTsMap !== 'object') this.state.inboxDmLastTsMap = {};
            const sinceOnly = !!(opts && opts.sinceOnly);
            const sinceId = sinceOnly ? Number(this.state.dmLastMsgIdMap[pid] || 0) : 0;
            const limit = sinceOnly ? 120 : 200;
            const url = `/api/v1/messages/list?user_id=${this.state.user.id}&other_id=${pid}&limit=${limit}${sinceId > 0 ? `&since_id=${sinceId}` : ''}`;
            ...
            let lastTs = sinceOnly ? Number(this.state.inboxDmLastTsMap[pid] || 0) : 0;
            msgs.forEach(m => {
                const ts = m.created_at ? new Date(m.created_at).getTime() : 0;
                if (ts && (!lastTs || Math.abs(ts - lastTs) > 5 * 60 * 1000)) {
                    const t = document.createElement('div');
                ...
                try {
                    const mid = Number(m && m.id ? m.id : 0);
                    if (mid && mid > Number(this.state.dmLastMsgIdMap[pid] || 0)) this.state.dmLastMsgIdMap[pid] = mid;
                    if (ts && ts > Number(this.state.inboxDmLastTsMap[pid] || 0)) this.state.inboxDmLastTsMap[pid] = ts;
                } catch (_) {
                }
            });
