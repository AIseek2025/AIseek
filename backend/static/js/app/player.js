Object.assign(window.app, {
    ...
    _escapeAttr: function(v) {
        return String(v || '').replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    },

    _normalizeSubtitleTracks: function(post) {
        const arr = post && Array.isArray(post.subtitle_tracks) ? post.subtitle_tracks : [];
        const out = [];
        for (const t of arr) {
            if (!t || typeof t !== 'object') continue;
            const lang = String(t.lang || '').trim();
            const label = String(t.label || lang || '中文字幕').trim();
            const url = String(t.url || '').trim();
            if (!url) continue;
            out.push({ lang: lang || 'zh', label, url, isDefault: !!t.is_default });
        }
        return out;
    },

    _subtitleTracksHtml: function(post) {
        const items = this._normalizeSubtitleTracks(post);
        if (!items.length) return '';
        return items.map((it, idx) => `<track kind="subtitles" srclang="${this._escapeAttr(it.lang)}" label="${this._escapeAttr(it.label)}" src="${this._escapeAttr(it.url)}"${(it.isDefault || idx === 0) ? ' default' : ''}>`).join('');
    },

    attachSubtitleTracksToVideo: function(video, post) {
        const v = video;
        if (!v) return;
        try {
            const old = Array.from(v.querySelectorAll('track'));
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            try {
                ...
                tr.kind = 'subtitles';
                tr.srclang = String(it.lang || 'zh');
                tr.label = String(it.label || it.lang || '中文字幕');
                tr.src = String(it.url || '');
                tr.default = !!(it.isDefault || i === 0);
                v.appendChild(tr);
            } catch (_) {}
        }
    ...
    _bindPosterFallback: function(video, slide) {
        ...
        const s = slide;
        if (!v || !s) return;
        const fb = s.querySelector('.video-poster-fallback');
        if (!fb) return;
        try {
            fb.style.transition = 'opacity .28s ease';
            fb.style.willChange = 'opacity';
        } catch (_) {}
        let firstPlayAt = 0;
        const hide = () => {
            try {
        ...
        v.addEventListener('loadeddata', () => {
            try {
                if (Number(v.currentTime || 0) > 0 || Number(v.readyState || 0) >= 2) hide();
            } catch (_) {}
        });
    ...
    _ensureCoverBadge: function(slide, post) {
        if (!slide) return;
        const stage = slide.querySelector('.video-player-container') || slide;
        if (!stage) return;
        if (!this._isMediaStampEnabled()) {
            try {
                const b = stage.querySelector('.video-cover-badge');
                if (b) b.remove();
                const t = stage.querySelector('.video-cover-badge-tag');
                if (t) t.remove();
            } catch (_) {}
            return;
        }
        const pid = Number((post && post.id) || 0);
        const cover = String((post && post.cover_url) || '').trim() || `/api/v1/media/post-thumb/${encodeURIComponent(String(pid || '0'))}?v=${Date.now()}`;
        let badge = stage.querySelector('.video-cover-badge');
        if (!badge) {
            badge = document.createElement('img');
            badge.className = 'video-cover-badge';
        ...
        badge.onerror = () => {
            try {
                const fb = `/api/v1/media/post-thumb/${encodeURIComponent(String(pid || '0'))}?_ts=${Date.now()}`;
                if (!String(badge.src || '').includes('/api/v1/media/post-thumb/')) badge.src = fb;
            } catch (_) {}
        };
        badge.src = cover;
        let tag = stage.querySelector('.video-cover-badge-tag');
        if (!tag) {
            tag = document.createElement('div');
            tag.className = 'video-cover-badge-tag';
            ...
            tag.style.color = '#fff';
            tag.style.background = 'rgba(0,0,0,.72)';
            tag.style.border = '1px solid rgba(255,255,255,.45)';
        ...
        tag.innerText = '封面';
        tag.style.opacity = '.86';
    },

    _normalizeSubtitleUrl: function(u) {
    ...
    _pickSubtitleUrl: function(post) {
        const items = this._normalizeSubtitleTracks(post);
        if (!items.length) return '';
        const byDefault = items.find(it => !!it.isDefault) || items[0];
        return this._normalizeSubtitleUrl(byDefault ? byDefault.url : '');
    },

    _parseVttCues: function(raw) {
        ...
        while (i < lines.length) {
            ...
            const body = seg.join('\n').trim();
            if (body && ed > st) cues.push({ st, ed, body });
            i += 1;
        }
        return cues;
    },
    ...
    _withTs: function(u) {
        const s = String(u || '').trim();
        if (!s) return s;
        if (s.includes('_ts=')) return s;
        const sep = s.includes('?') ? '&' : '?';
        return `${s}${sep}_ts=${Date.now()}`;
    },

    _normalizeMediaUrl: function(u) {
        const s = String(u || '').trim();
        if (!s) return '';
        if (/^https?:\/\//i.test(s)) return s;
        if (s.startsWith('/')) return s;
    ...
    refreshPostMediaForPlayback: async function(post, slide, video) {
        const p = post || {};
        const pid = Number(p.id || 0);
        if (!pid) return p;
        const uid = Number((this.state && this.state.user && this.state.user.id) || 0);
        const q = uid ? `?user_id=${uid}&_ts=${Date.now()}` : `?_ts=${Date.now()}`;
        const url = `/api/v1/posts/${pid}${q}`;
        try {
            const res = await this.apiRequest('GET', url, undefined, { cache: 'no-store' });
            if (!res.ok) return p;
            const latest = await res.json();
            if (!latest || typeof latest !== 'object') return p;
            p.video_url = this._normalizeMediaUrl(latest.video_url || p.video_url || '');
            p.hls_url = this._normalizeMediaUrl(latest.hls_url || p.hls_url || '');
            p.mp4_url = this._normalizeMediaUrl(latest.mp4_url || p.mp4_url || '');
            p.cover_url = this._normalizeMediaUrl(latest.cover_url || p.cover_url || '');
            p.subtitle_tracks = Array.isArray(latest.subtitle_tracks) ? latest.subtitle_tracks : (p.subtitle_tracks || []);
            p.media_version = String(latest.media_version || p.media_version || '');
            p.ai_job_id = String(latest.ai_job_id || p.ai_job_id || '');
            if (video) {
                const cv = String(p.cover_url || '').trim();
                if (cv) {
                    try { video.poster = this._withTs(cv); } catch (_) {}
                    const fb = slide ? slide.querySelector('.video-poster-fallback') : null;
                    if (fb) {
                        try {
                            const primary = this._withTs(cv);
                            const fallback = `/api/v1/media/post-thumb/${encodeURIComponent(String(pid))}?_ts=${Date.now()}`;
                            fb.onerror = () => {
                                try { if (!String(fb.src || '').includes('/api/v1/media/post-thumb/')) fb.src = fallback; } catch (_) {}
                            };
                            fb.src = primary;
                        } catch (_) {}
                    }
    ...
    _ensureMediaStamp: function(slide, post) {
        if (!slide) return;
        const enabled = this._isMediaStampEnabled();
        const pid = Number((post && post.id) || 0);
        if (!(pid === 77 || pid === 80)) return;
        const stage = slide.querySelector('.video-player-container') || slide;
        if (!stage) return;
        let tag = stage.querySelector('.video-media-stamp');
        if (!enabled) {
            if (tag) {
        ...
        if (!tag) {
            tag = document.createElement('div');
            tag.className = 'video-media-stamp';
            tag.style.position = 'absolute';
            ...
            tag.style.color = 'rgba(255,255,255,.86)';
            tag.style.background = 'rgba(0,0,0,.45)';
            tag.style.border = '1px solid rgba(255,255,255,.24)';
            tag.style.borderRadius = '8px';
            tag.style.zIndex = '26';
            tag.style.pointerEvents = 'none';
            stage.appendChild(tag);
        }
        const mv = String((post && post.media_version) || '');
        const jid = String((post && post.ai_job_id) || '').slice(0, 8);
        tag.innerText = `P${pid} M${mv || '-'} J${jid || '-'}`;
    },
    ...
    _applySubtitleVisualStyle: function(box) {
        const el = box;
        if (!el) return;
    ...
    _ensureSubtitleOverlay: function(video, slide, post) {
        if (!video || !slide) return;
        let box = slide.querySelector('.video-subtitle-overlay');
        if (!box) {
            box = document.createElement('div');
            box.className = 'video-subtitle-overlay';
        ...
        const cached = this.state._subtitleCueCache[cacheKey];
        if (Array.isArray(cached)) {
            bindTicker(cached);
            return;
        }
        const q = subUrl.indexOf('?') >= 0 ? '&' : '?';
        const subFullUrl = `${subUrl}${q}_ts=${Date.now()}`;
        this.apiRequest('GET', subFullUrl, undefined, { cache: 'no-store' })
            .then(r => r.ok ? r.text() : '')
            .then(txt => {
                const cues = this._parseVttCues(txt);
                this.state._subtitleCueCache[cacheKey] = cues;
                bindTicker(cues);
            })
            .catch(() => {
                bindTicker([]);
    ...
    applyPreferredVideoSource: async function(video, post, opts = {}) {
        ...
        const targetRaw = (hlsUrl && canNativeHls) ? hlsUrl : (hlsUrl || mp4Url || '');
        const target = this._withTs(targetRaw);
        if (!target) return;
                    ...
                    hls.on(Hls.Events.ERROR, async (_, data) => {
                        try {
                            if (data && data.fatal && mp4Url) {
                                ...
                                v.src = this._withTs(mp4Url);
                                try { v.load(); } catch (_) {}
                                try { if (opts && opts.autoPlay) await v.play(); } catch (_) {}
                            }
                        } catch (_) {}
                    });
                    hls.loadSource(this._withTs(hlsUrl));
                    hls.attachMedia(v);
                    return;
                }
            } catch (_) {
    ...
    renderVideoSlide: function(post, opts = {}) {
        ...
        const pid = post && post.id ? String(post.id) : '0';
        const poster = (post && post.cover_url) ? String(post.cover_url).trim() : (pid ? `/api/v1/media/post-thumb/${encodeURIComponent(pid)}?v=${Date.now()}` : '');
        ...
        const subtitlesHtml = this._subtitleTracksHtml(post);
        const media = (isVideo && (isReady || canPlayNow))
            ? `
                <video id="${vidId}" data-hls="${hlsUrl}" data-mp4="${mp4Url}"${posterAttr}${arAttr} loop playsinline webkit-playsinline muted autoplay preload="metadata">${subtitlesHtml}</video>
                ${poster ? `<img class="video-poster-fallback" src="${poster}" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover; opacity:1; transition:opacity .25s ease; pointer-events:none; z-index:2;" alt="">` : ''}
                ${isReady ? '' : `
    ...
    bindVideoEvents: function(slide, post) {
        ...
        try {
            this.attachSubtitleTracksToVideo(video, post);
        } catch (_) {
        }
        try {
            this._bindPosterFallback(video, slide);
        } catch (_) {
        }
        try {
            this._ensureCoverBadge(slide, post);
        } catch (_) {
        }
        try {
            this._ensureSubtitleOverlay(video, slide, post);
        } catch (_) {
        }
        try {
            this._ensureMediaStamp(slide, post);
        } catch (_) {
        }
        this.refreshPostMediaForPlayback(post, slide, video).then((latestPost) => {
            try {
                this.applyPreferredVideoSource(video, latestPost || post, { autoPlay: false });
            } catch (_) {}
            try {
                this.attachSubtitleTracksToVideo(video, latestPost || post);
            } catch (_) {}
            try {
                this._ensureCoverBadge(slide, latestPost || post);
            } catch (_) {}
            try {
                this._ensureSubtitleOverlay(video, slide, latestPost || post);
            } catch (_) {}
            try {
                this._ensureMediaStamp(slide, latestPost || post);
            } catch (_) {}
        }).catch(() => {});
