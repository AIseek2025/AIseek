Object.assign(window.app, {
    ...
    toggleLike: async function(postId, btn) {
        ...
        try {
            const res = await fetch('/api/v1/interaction/like', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ post_id: postId, user_id: this.state.user.id })
            });
            ...
            if (txt) txt.innerText = data.count;
            if (window.appInteractions && typeof window.appInteractions.applyPostPatch === 'function') {
                window.appInteractions.applyPostPatch(postId, { is_liked: data.status === 'liked', likes_count: data.count });
            }
        } catch(e) {}
    },
    ...
    toggleFavorite: async function(postId, btn) {
        ...
        try {
            const res = await fetch('/api/v1/interaction/favorite', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ post_id: postId, user_id: this.state.user.id })
            });
            const data = await res.json();
    ...
    toggleRepost: async function(postId, btn) {
        ...
        try {
            const res = await fetch('/api/v1/interaction/repost', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ post_id: postId, user_id: this.state.user.id })
            });
            const data = await res.json();
    ...
    toggleFollow: async function(targetId, btn) {
        ...
        try {
            const res = await fetch('/api/v1/users/follow', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ user_id: this.state.user.id, target_id: targetId })
            });
            const data = await res.json();
            if (!res.ok) {
                ...
                return alert(data.detail || data.message || '操作失败');
            }
    ...
    appendComments: function(postId, comments) {
        ...
        arr.forEach(c => {
            ...
            metaSpan.innerText = `${this.fmtTimeAgo((c.created_at || 0) * 1000)} · ${c.location || '未知'}`;
    ...
    reactComment: async function(commentId, reaction, el) {
        if (!this.state.user) return this.openModal('authModal');
        try {
            const res = await fetch('/api/v1/interaction/comment/react', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ comment_id: commentId, user_id: this.state.user.id, reaction })
            });
            const data = await res.json();
            if (!res.ok) return;
            ...
            if (row) {
                ...
                if (likeEl) likeEl.innerText = data.likes_count ?? likeEl.innerText;
                if (dislikeEl) dislikeEl.innerText = data.dislikes_count ?? dislikeEl.innerText;
            }
        } catch (e) {}
    },
    ...
    sendDanmaku: function(postId, text) {
        ...
        if (this.state.user) {
            fetch('/api/v1/interaction/danmaku', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ post_id: postId, user_id: this.state.user.id, content: text, timestamp: ts })
            }).then(() => {
    ...
    loadDanmaku: async function(postId, force = false) {
        ...
        try {
            const res = await fetch(`/api/v1/interaction/danmaku/${postId}`);
    ...
    postComment: async function() {
        ...
        try {
            const res = await fetch('/api/v1/interaction/comment', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ 
                    post_id: postId, 
            ...
            if (res.ok) {
                input.value = '';
                this.clearCommentReply();
                this.openComments(postId); // Reload
            }
        } catch(e) { alert('发送失败'); }
    },
