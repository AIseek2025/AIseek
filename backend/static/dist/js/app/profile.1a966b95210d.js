Object.assign(window.app, {
    ...
    loadFriendsPage: async function() {
        ...
        panel.innerHTML = '<div style="padding:20px; text-align:center; color:#666;">加载中...</div>';
        if (main) main.innerHTML = '<div style="color:var(--text-secondary);">加载中...</div>';
        ...
        try {
            ...
            this.state.friendsList = friends.map(f => f.friend_user || f);
            
            this.renderFriendList(panel, main, this.state.friendsList, '好友', false);
            this.ensureFriendsDefaultOpen(panel, main, this.state.friendsList);
            
        } catch(e) {
            console.error(e);
            panel.innerHTML = '<div style="padding:20px; color:#666;">加载失败</div>';
            if (main) main.innerHTML = '<div style="color:var(--text-secondary);">加载失败</div>';
        }
    },
    ...
    loadUserWorkInMainPanel: async function(userId, container) {
        ...
        try {
            ...
            posts.forEach(post => {
                ...
                slide.id = `friend-slide-${userId}-${post.id}`;
    ...
    loadProfile: async function(userId) {
        ...
        try {
            ...
            if (isMe) {
                ...
                const dmBtn = `<button class="btn-primary" style="width:88px; height:34px; padding:0; background:#333; margin-left:8px; font-size:14px;" data-action="call" data-fn="startChat" data-args='[${user.id}, ${JSON.stringify(user.nickname||user.username)}]'>私信</button>`;
            ...
            header.innerHTML = `
                ...
                <div class="profile-header-inner">
                    ...
                    <div class="profile-info-box">
                        ...
                        <div class="profile-id" title="信誉分越高，社区互动体验越好">信誉分：<b style="color:rgba(255,255,255,0.9);">${(user.reputation_score==null?100:user.reputation_score)}</b></div>
    ...
    _loadAIChat: async function(jobId) {
        ...
        try {
            const url = `/api/v1/ai/jobs/${encodeURIComponent(jid)}/chat?user_id=${uid}&limit=120`;
    ...
    uploadBackground: async function(input) {
        ...
        const formData = new FormData();
        formData.append('file', input.files[0]);
        const res = await fetch('/api/v1/upload/background', { method: 'POST', body: formData });
        const data = await res.json();
        if (data.url) {
            const bgEl = document.getElementById('edit_bg_img');
    ...
    changePassword: async function() {
        ...
        try {
            const res = await fetch('/api/v1/users/change-password', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ user_id: this.state.user.id, old_password: oldP, new_password: newP })
            });
            const data = await res.json();
            alert(data.message);
            if (res.ok) this.closeModal('passwordModal');
        } catch(e) { alert('修改失败'); }
    },
    ...
    updateContactInSettings: async function() {
        ...
        try {
            const res = await fetch('/api/v1/users/update-contact', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ user_id: this.state.user.id, phone, email })
            });
            const data = await res.json();
            if (!res.ok) return alert(data.detail || '保存失败');
            this.state.user = data.user;
            this.updateAuthUI();
            this.settingsSelect('center');
        } catch (_) {
            alert('保存失败');
        }
    },
    ...
    changePasswordInSettings: async function() {
        ...
        try {
            ...
            const data = await res.json();
            if (!res.ok) return alert(data.detail || '修改失败');
