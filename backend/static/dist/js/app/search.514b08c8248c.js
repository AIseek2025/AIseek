Object.assign(window.app, {
    ...
    searchUser: async function() {
        ...
        // Switch to Search Page inside Content Area
    ...
    renderSearchResults: async function() {
        ...
        try {
            ...
            if (mode === 'video') {
                ...
                const url = `/api/v1/posts/search?query=${encodeURIComponent(query)}${viewer ? `&user_id=${viewer}` : ''}`;
                ...
                const url = `/api/v1/users/search-user?query=${encodeURIComponent(query)}`;
                ...
                const viewer = this.state.user ? this.state.user.id : '';
                const uUrl = `/api/v1/users/search-user?query=${encodeURIComponent(query)}`;
                const pUrl = `/api/v1/posts/search?query=${encodeURIComponent(query)}${viewer ? `&user_id=${viewer}` : ''}`;
                const [uRes, pRes] = await Promise.all([
                    this.apiRequest('GET', uUrl, undefined, { cancel_key: `search:all:users`, dedupe_key: uUrl }),
                    this.apiRequest('GET', pUrl, undefined, { cancel_key: `search:all:posts`, dedupe_key: pUrl })
                ]);
                if (!uRes.ok) throw new Error(`GET ${uUrl} ${uRes.status}`);
                if (!pRes.ok) throw new Error(`GET ${pUrl} ${pRes.status}`);
                ...
                if (users.length === 0 && posts.length === 0) {
                    ...
                    if (Array.isArray(posts) && posts.length > 0) {
                        ...
                        html += `<div class="search-waterfall">${renderVideoCards(posts)}</div>`;
                    }
                }
            }
            grid.innerHTML = html;
