Object.assign(window.app, {
    ensureFloatingPlayer: function() {
        ...
        try {
            ...
            st.textContent = '#floating_player:hover #floating_player_overlay,#floating_player:hover #floating_btn_close{opacity:1 !important;pointer-events:auto !important;}';
        } catch (_) {
        }
    ...
    openFloatingPlayer: async function(postId, opts = {}) {
        ...
        if (opts && opts.preset === 'jx') {
            ...
            let maxW = Math.max(minW, window.innerWidth);
            let maxH = Math.max(minH, window.innerHeight);
            maxH = Math.max(minH, Math.min(maxH, maxW / ratio));
            maxW = Math.max(minW, Math.min(maxW, maxH * ratio));
            let w = Math.min(maxW, minW * 2);
            let h = w / ratio;
            h = Math.max(minH, Math.min(maxH, h));
            w = h * ratio;
            wrap.style.width = `${w}px`;
            wrap.style.height = `${h}px`;
            wrap.dataset.jxMinW = String(minW);
            wrap.dataset.jxMinH = String(minH);
        ...
        if (opts && opts.preset === 'jx' && opts.anchorEl && opts.anchorEl.getBoundingClientRect) {
            ...
            const fits = (left, top) => left >= 0 && top >= 0 && (left + bw) <= window.innerWidth && (top + bh) <= window.innerHeight;

            const rightLeft = r.right + gap;
            const leftLeft = r.left - bw - gap;
            const bottomTop = r.bottom + gap;
            const topTop = r.top - bh - gap;

            const topForSide = clamp01(r.top, 0, Math.max(0, window.innerHeight - bh));
            const leftForTB = clamp01(r.left, 0, Math.max(0, window.innerWidth - bw));

            const candidates = [
                { left: rightLeft, top: topForSide },
                { left: leftLeft, top: topForSide },
                { left: leftForTB, top: bottomTop },
                { left: leftForTB, top: topTop },
            ];
            const pick = candidates.find(p => fits(p.left, p.top)) || candidates[0];
            wrap.style.left = `${pick.left}px`;
            wrap.style.top = `${pick.top}px`;
            wrap.style.right = 'auto';
            wrap.style.bottom = 'auto';
    ...
    floatingDragEnd: function(ev) {
        ...
        try { if (wrap._floatingClamp) wrap._floatingClamp(); } catch (_) {}
        try { if (wrap._floatingSaveRect) wrap._floatingSaveRect(); } catch (_) {}
    },
    ...
    floatingResizeEnd: function(handleEl, dir, ev) {
        ...
        try { if (wrap) wrap.style.cursor = 'grab'; } catch (_) {}
        try { if (wrap && wrap._floatingClamp) wrap._floatingClamp(); } catch (_) {}
        try { if (wrap && wrap._floatingSaveRect) wrap._floatingSaveRect(); } catch (_) {}
    },
    ...
    toggleFloatingPlayerSize: function() {
        ...
        if (wrap._floatingClamp) wrap._floatingClamp();
        if (wrap._floatingSaveRect) wrap._floatingSaveRect();
    }
});
