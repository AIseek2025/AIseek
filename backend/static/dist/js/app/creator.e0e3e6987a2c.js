Object.assign(window.app, {
    creatorSelectTab: function(tab) {
        // Toggle tabs
        document.getElementById('creator_tab_manual').className = `creator-tab2 ${tab === 'manual' ? 'active' : ''}`;
        document.getElementById('creator_tab_ai').className = `creator-tab2 ${tab === 'ai' ? 'active' : ''}`;
    ...
    creatorSubmitManual: async function() {
        ...
        try {
            ...
            const tagList = String(tags || '').split(/[\s,，#]+/).filter(Boolean).slice(0, 12);
            ...
            const res2 = await fetch('/api/v1/posts/create', {
                method: 'POST',
    ...
    creatorSubmitAI: async function() {
        ...
        try {
            const res = await fetch('/api/v1/ai/submit', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({
                    user_id: this.state.user.id,
