Object.assign(window.app, {
    ...
    loadHeaderDropdowns: async function() {
        ...
        // 1. Notifications
        try {
            const res = await fetch(`/api/v1/interaction/notifications/${this.state.user.id}`);
        ...
        try {
            const res = await fetch(`/api/v1/interaction/notifications_unread/${this.state.user.id}`);
            if (res.ok) {
        ...
        // 2. DMs
        try {
            const res = await fetch(`/api/v1/messages/conversations?user_id=${this.state.user.id}`);
    ...
    loadUserHoverStats: async function() {
        ...
        try {
            ...
            const [likesRes, favRes, worksRes] = await Promise.all([
                fetch(`/api/v1/interaction/likes/${uid}`),
                fetch(`/api/v1/interaction/favorites/${uid}`),
                fetch(`/api/v1/posts/user/${uid}`)
            ]);
    ...
    startChat: function(targetUserId, nickname) {
        const action = { type: 'open_inbox', tab: 'dm', peer_id: targetUserId, peer_meta: { nickname: nickname || ('用户' + targetUserId) } };
        if (typeof this.ensureAuth === 'function') {
            if (!this.ensureAuth(action)) return;
        } else if (!this.state.user) {
            this.state.pendingAuthAction = action;
        ...
        this.state.inboxPendingPeer = { id: targetUserId, meta: { nickname: nickname || ('用户' + targetUserId) } };
        this.openInbox('dm');
    },
    ...
    dmSearchUsers: async function(query) {
        ...
        try {
            const res = await fetch(`/api/v1/users/search-user?query=${encodeURIComponent(q)}`);
    ...
    loadDMConversations: async function() {
        ...
        try {
            if (!this.state.dmFriendIdsLoadedAt || (Date.now() - Number(this.state.dmFriendIdsLoadedAt || 0)) > 15000) {
                const res = await fetch(`/api/v1/users/friends/list?user_id=${this.state.user.id}`);
    ...
    dmSendImage: async function(fileInput) {
        ...
        try {
            ...
            fd.append('file', f);
            const up = await fetch('/api/v1/upload/local', { method: 'POST', body: fd });
            ...
            const res = await fetch('/api/v1/messages/send', {
                ...
                body: JSON.stringify({ sender_id: this.state.user.id, receiver_id: peerId, content })
            });
    ...
    loadDMThread: async function(peerId) {
        ...
        try {
            const res = await fetch(`/api/v1/messages/list?user_id=${this.state.user.id}&other_id=${peerId}`);
    ...
    sendDMMessage: async function() {
        ...
        try {
            const res = await fetch('/api/v1/messages/send', {
                method: 'POST', headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ sender_id: this.state.user.id, receiver_id: this.state.dmPeerId, content })
            });
            if (!res.ok) return;
    ...
    loadInboxNotifications: async function() {
        ...
        try {
            const [unreadRes, listRes] = await Promise.all([
                fetch(`/api/v1/interaction/notifications_unread/${this.state.user.id}`),
                this.apiRequest('GET', `/api/v1/interaction/notifications/${this.state.user.id}?limit=200`)
            ]);
