Object.assign(window.app, {
    ...
    openFloatingPlayer: async function(postId, opts = {}) {
        ...
        if (opts && opts.preset === 'jx' && opts.anchorEl && opts.anchorEl.getBoundingClientRect) {
            ...
            const left = preferRight ? (window.innerWidth - bw - pad) : pad;
            const top = headerH + pad;
            wrap.style.left = `${Math.max(pad, Math.min(window.innerWidth - bw - pad, left))}px`;
            wrap.style.top = `${Math.max(pad, Math.min(window.innerHeight - bh - pad, top))}px`;
            wrap.style.right = 'auto';
            wrap.style.bottom = 'auto';
