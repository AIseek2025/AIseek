    ...
    attachSubtitleTracksToVideo: function(video, post) {
        ...
        try {
            ...
            for (const it of items) {
                if (it.isDefault) {
                    target = String(it.lang || '');
                    break;
                }
            }
            if (!target && items[0]) target = String(items[0].lang || '');
            for (let i = 0; i < tracks.length; i++) {
                const tr = tracks[i];
                try {
                    tr.mode = (target && String(tr.language || '') === target) ? 'showing' : 'disabled';
                } catch (_) {}
            }
        } catch (_) {}
    },
    ...
    renderVideoSlide: function(post, opts = {}) {
        ...
        const media = (isVideo && (isReady || canPlayNow))
            ? `
                ...
                ${poster ? `<img class="video-poster-fallback" src="${poster}" style="position:absolute; inset:0; width:100%; height:100%; object-fit:cover; opacity:1; transition:opacity .25s ease; pointer-events:none;">` : ''}
                ${isReady ? '' : `
