Object.assign(window.app, {
    ensureFloatingPlayer: function() {
        ...
        const mkHandle = (dir, css) => {
            ...
            if (dir === 'se') {
                h.style.width = '28px';
                h.style.height = '28px';
                h.style.borderRadius = '9px';
                h.style.display = 'block';
                h.style.background = 'rgba(255,255,255,0.20)';
                h.style.border = '1px solid rgba(255,255,255,0.24)';
                h.style.boxShadow = '0 10px 22px rgba(0,0,0,0.45)';
                h.style.display = 'flex';
                h.style.alignItems = 'center';
                h.style.justifyContent = 'center';
                h.innerHTML = '<i class="fas fa-expand-arrows-alt" style="color: rgba(255,255,255,0.9); font-size: 12px;"></i>';
            }
            try {
                h.dataset.elPos = '0';
                h.dataset.stop = '1';
        ...
        const restore = () => {
            try {
                ...
                const maxH = Math.max(135, window.innerHeight - pad);
                if (Number.isFinite(r.w) && Number.isFinite(r.h)) {
                    wrap.style.width = `${Math.max(240, Math.min(maxW, r.w))}px`;
                    wrap.style.height = `${Math.max(135, Math.min(maxH, r.h))}px`;
                }
                if (Number.isFinite(r.x) && Number.isFinite(r.y)) {
                    wrap.style.left = `${r.x}px`;
                    wrap.style.top = `${Math.max(0, Math.min(window.innerHeight - 24, r.y))}px`;
        ...
        const syncSizeIcon = () => {
            const i = btnSize.querySelector('i');
            if (!i) return;
            const mode = String(wrap.dataset.sizeMode || '');
            i.className = mode === 'fill' ? 'fas fa-compress' : 'fas fa-expand';
        };
        ...
        document.addEventListener('fullscreenchange', syncSizeIcon);
        document.addEventListener('fullscreenchange', () => {
            try {
                if (!document.fullscreenElement && String(wrap.dataset.sizeMode || '') === 'fill' && wrap._floatingExitFill) {
                    wrap._floatingExitFill();
                }
            } catch (_) {
            }
        });
        ...
        const clamp = () => {
            if (String(wrap.dataset.sizeMode || '') === 'fill') return;
            ...
            const minY = Math.max(0, headerH + pad);
            const x0 = Math.max(pad, Math.min(maxX - pad, rect.left));
            const y0 = Math.max(minY, Math.min(maxY - pad, rect.top));
            const isJx = String(wrap.dataset.resizeEnabled || '') === '1' || String(wrap.dataset.noPersist || '') === '1' || String(wrap.dataset.preset || '') === 'jx';
            const snap = 36;
            let x = x0;
            let y = y0;
            if (!isJx) {
                if (x <= snap) x = pad;
                if (x >= (maxX - snap)) x = Math.max(pad, maxX - pad);
            }
    ...
    openFloatingPlayer: async function(postId, opts = {}) {
        ...
        if (opts && opts.preset === 'jx') {
            try { wrap.dataset.preset = 'jx'; } catch (_) {}
            try { wrap.dataset.pinned = '0'; } catch (_) {}
            let ratio = 16 / 9;
            let minW = 320;
            const headerH = (() => {
                try {
            ...
            const maxW = Math.max(240, Math.min(window.innerWidth - 24, 380));
            const maxH = Math.max(135, Math.min(window.innerHeight - headerH - 24, 214));
            ...
            try {
                ...
                const pad = 16;
                wrap.dataset.jxMinW = '240';
                wrap.dataset.jxMinH = '135';
                wrap.dataset.jxMaxW = String(Math.max(240, window.innerWidth - pad));
                wrap.dataset.jxMaxH = String(Math.max(135, window.innerHeight - headerH - pad));
                wrap.dataset.resizeEnabled = '1';
                wrap.dataset.noPersist = '0';
                const hs = wrap.querySelectorAll('.floating-resize-handle');
                Array.from(hs).forEach((x) => {
                    const cls = String(x.className || '');
                    const show = cls.includes('floating-resize-se');
                    try { x.style.display = show ? 'flex' : 'none'; } catch (_) {}
                });
            } catch (_) {
            }
        }
        ...
        if (opts && opts.preset === 'jx') {
            const baseRect = (() => {
                try {
                    if (opts && opts.anchorEl && opts.anchorEl.getBoundingClientRect) return opts.anchorEl.getBoundingClientRect();
                } catch (_) {
                }
                try {
                    if (srcVideo && srcVideo.getBoundingClientRect) return srcVideo.getBoundingClientRect();
                } catch (_) {
                }
                return null;
            })();
            ...
            const clamp = (v, a, b) => Math.max(a, Math.min(b, v));
            const minY = Math.max(0, headerH + pad);
            const maxY = Math.max(minY, window.innerHeight - bh - pad);

            let left = window.innerWidth - bw - pad;
            let top = minY;
            if (baseRect) {
                const r = baseRect;
                const roomRight = Math.max(0, window.innerWidth - Number(r.right || 0));
                const roomLeft = Math.max(0, Number(r.left || 0));
                const preferRight = roomRight >= roomLeft;
                const canRight = roomRight >= (bw + gap + pad);
                const canLeft = roomLeft >= (bw + gap + pad);
                if (preferRight && canRight) left = Number(r.right || 0) + gap;
                else if (!preferRight && canLeft) left = Number(r.left || 0) - bw - gap;
                else if (canRight) left = Number(r.right || 0) + gap;
                else if (canLeft) left = Number(r.left || 0) - bw - gap;
                else left = preferRight ? (window.innerWidth - bw - pad) : pad;
                top = clamp(Number(r.top || 0), minY, maxY);
            }
            wrap.style.left = `${clamp(left, pad, Math.max(pad, window.innerWidth - bw - pad))}px`;
            ...
            wrap.style.right = 'auto';
            wrap.style.bottom = 'auto';
        } else if (!opts || !opts.preset) {
            try {
                const v = srcVideo && srcVideo.getBoundingClientRect ? srcVideo.getBoundingClientRect() : null;
                if (v) {
                    const bw = wrap.getBoundingClientRect().width;
                    const bh = wrap.getBoundingClientRect().height;
                    const headerH = (() => {
                        try {
                            const vv = getComputedStyle(document.documentElement).getPropertyValue('--header-height');
                            const n = parseFloat(String(vv || '').replace('px', '').trim());
                            return Number.isFinite(n) ? n : 56;
                        } catch (_) {
                            return 56;
                        }
                    ...
                    const roomLeft = Number(v.left || 0);
                    const roomRight = Math.max(0, window.innerWidth - Number(v.right || 0));
                    const left = (roomRight >= roomLeft) ? (window.innerWidth - bw - pad) : pad;
                    const top = clamp(Number(v.top || 0), headerH + pad, Math.max(headerH + pad, window.innerHeight - bh - pad));
        ...
        try {
            const coverUrl = (post && post.cover_url) ? String(post.cover_url).trim() : '';
            const fallbackCover = pid ? `/api/v1/media/post-thumb/${encodeURIComponent(String(pid))}?v=${Date.now()}` : '';
            const finalCover = coverUrl || fallbackCover;
            if (finalCover) {
                video.poster = finalCover;
            } else {
        ...
        try {
            if (post && window.app && typeof window.app.attachSubtitleTracksToVideo === 'function') {
                window.app.attachSubtitleTracksToVideo(video, post);
            }
        } catch (_) {
        }
        try {
    ...
    floatingDragEnd: function(ev) {
        ...
        if (!wrap) return;
        s.active = false;
        try { wrap.dataset.pinned = '1'; } catch (_) {}
    ...
    closeFloatingPlayer: function() {
        ...
        const video = document.getElementById('floating_player_video');
        try {
            if (wrap && String(wrap.dataset.sizeMode || '') === 'fill' && wrap._floatingExitFill) wrap._floatingExitFill();
        } catch (_) {
        }
        if (video) {
        ...
        try {
            if (document.fullscreenElement && wrap && document.fullscreenElement === wrap) {
                document.exitFullscreen().catch(() => {});
            }
        } catch (_) {
        }
    ...
    toggleFloatingPlayerSize: function() {
        ...
        const icon = btn ? btn.querySelector('i') : null;
        const ensureFillFns = () => {
            if (wrap._floatingEnterFill && wrap._floatingExitFill) return;
            wrap._floatingEnterFill = () => {
                try {
                    const rect = wrap.getBoundingClientRect();
                    wrap._floatingPrevRect = { left: rect.left, top: rect.top, width: rect.width, height: rect.height, br: String(wrap.style.borderRadius || '') };
                } catch (_) {
                    wrap._floatingPrevRect = null;
                }
                try { wrap.dataset.sizeMode = 'fill'; } catch (_) {}
                try { wrap.dataset.pinned = '1'; } catch (_) {}
                try {
                    wrap.style.left = '0px';
                    wrap.style.top = '0px';
            ...
            wrap._floatingExitFill = () => {
                try { wrap.dataset.sizeMode = ''; } catch (_) {}
                const prev = wrap._floatingPrevRect;
                if (prev && typeof prev === 'object') {
                    try {
                        wrap.style.width = `${Number(prev.width || 360)}px`;
                        wrap.style.height = `${Number(prev.height || 202)}px`;
                        wrap.style.left = `${Number(prev.left || 8)}px`;
                        wrap.style.top = `${Number(prev.top || 8)}px`;
                        wrap.style.right = 'auto';
                        wrap.style.bottom = 'auto';
                    } catch (_) {
                    }
                    try { wrap.style.borderRadius = String(prev.br || '12px') || '12px'; } catch (_) {}
                } else {
                    try { wrap.style.borderRadius = '12px'; } catch (_) {}
                    try { wrap.style.right = '8px'; wrap.style.bottom = '18px'; wrap.style.left = 'auto'; wrap.style.top = 'auto'; } catch (_) {}
                }
                try { wrap._floatingPrevRect = null; } catch (_) {}
                try { if (wrap._floatingClamp) wrap._floatingClamp(); } catch (_) {}
                try {
        ...
        const mode = String(wrap.dataset.sizeMode || '');
        if (mode === 'fill') {
            try { if (wrap._floatingExitFill) wrap._floatingExitFill(); } catch (_) {}
            try { if (icon) icon.className = 'fas fa-expand'; } catch (_) {}
            try {
                if (String(wrap.dataset.noPersist || '') !== '1' && wrap._floatingSaveRect) wrap._floatingSaveRect();
            } catch (_) {}
            return;
        ...
        try { if (wrap._floatingEnterFill) wrap._floatingEnterFill(); } catch (_) {}
        try { if (icon) icon.className = 'fas fa-compress'; } catch (_) {}
    }
});
