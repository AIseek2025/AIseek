...
def _runtime_file() -> Path:
    ...
    return root / "runtime" / "client_events.json"


def read_runtime_client_events() -> Dict[str, Any]:
    try:
...
def write_runtime_client_events(cfg: Dict[str, Any]) -> None:
...
def patch_runtime_client_events(delta: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for k, v in (delta or {}).items():
        if v is None:
            continue
        out[str(k)] = v
    write_runtime_client_events(out)
    return out
